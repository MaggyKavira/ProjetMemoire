...
return render_template('index.html', produits_mieux_notes=produits_mieux_notes,nb_clients=nb_clients,nb_produits=nb_produits,nb_transactions=nb_transactions)
...
# Modifier un client - affichage du formulaire
@app.route('/modifier_client/<int:client_id>')
def modifier_client(client_id):
    client = Client.query.get_or_404(client_id)
    return render_template('modifier_client.html', client=client)

# Modifier un client - traitement du formulaire
...
def enregistrer_modification_client(client_id):
    ...
    client.preferences = request.form['preferences'].strip()
    db.session.commit()
    # Ajoute un message flash si tu veux
    return redirect(url_for('clients'))
...
def supprimer_client(client_id):
    client = Client.query.get_or_404(client_id)
    if client.transactions:  # Vérifie grâce au relationship/backref dans le modèle
        flash("Suppression impossible : ce client possède encore des transactions dans l’historique.", "danger")
        return redirect(url_for('clients'))
    db.session.delete(client)
    db.session.commit()
...
def ajouter_produit():
    ...
    # Vérification d'unicité (nom + catégorie)
    produit_existant = Produit.query.filter_by(nom=nom, categorie=categorie).first()
    if produit_existant:
        flash(f"Le produit '{nom}' dans la catégorie '{categorie}' existe déjà.", "warning")
    ...
    try:
        ...
        db.session.rollback()
        flash(f"Le produit '{nom}' dans la catégorie '{categorie}' existe déjà (erreur d'intégrité).", "warning")
    return redirect(url_for('produits'))'''
    ...
    try:
        ...
        db.session.rollback()
        flash(f"Erreur : Le produit '{nom}' dans la catégorie '{categorie}' existe déjà (base).", "danger")
